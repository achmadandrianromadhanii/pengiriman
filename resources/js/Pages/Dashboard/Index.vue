...
<template>
    ...
    <div class="hidden md:block space-y-6 animate-fade-in">
        ...
        <div class="grid grid-cols-2 xl:grid-cols-4 gap-5">
            <StatCard title="Total Pengiriman" icon="bi-box-seam" :value="Number(stats.totalPengiriman || 0)"
                format="number" topBorderClass="bg-primary" :delta-text="pengirimanDeltaText"
                :delta-tone="pengirimanDeltaTone" />

            <StatCard title="Total Pendapatan" icon="bi-cash-coin" :value="Number(stats.totalPendapatan || 0)"
                format="rupiah" topBorderClass="bg-emerald-500" :delta-text="pendapatanDeltaText"
                :delta-tone="pendapatanDeltaTone" />

            <StatCard title="Success Rate" icon="bi-bullseye" :value="Number(stats.successRate || 0)" format="percent"
                topBorderClass="bg-blue-500" delta-text="Persentase pesanan sukses" delta-tone="neutral" />

            <StatCard title="Paket Kendala" icon="bi-exclamation-triangle" :value="Number(stats.paketBermasalah || 0)"
                format="number" topBorderClass="bg-red-500" delta-text="Terlambat / Gagal / Batal" delta-tone="neg" />
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-5 gap-5">
            ...
            <div class="glass card-hover xl:col-span-3 p-6 overflow-hidden">
                <div class="flex items-center justify-between mb-4">
                    <div class="font-heading font-extrabold text-lg text-gray-900 dark:text-white truncate">
                ...
                <div v-else class="w-full">
                    <ApexAreaChart :labels="charts.area.labels" :seriesRevenue="charts.area.seriesRevenue"
        ...
        <div class="glass card-hover p-6">
            ...
            <div class="flex items-center justify-between gap-3 mt-5">
                ...
                <div class="flex flex-wrap gap-2">
                    <a v-for="(l, i) in latest.links" :key="i" :href="l.url || '#'"
                        class="px-3 py-2 rounded-xl border text-sm transition" :class="l.active
                            ...
                            " v-html="cleanPaginationLabel(l.label)" @click.prevent="
                            l.url &&
                            router.visit(l.url, {
                                preserveScroll: true,
                                preserveState: true,
            ...
            <div class="mt-4 flex justify-end">
                <button class="btn-secondary" type="button" @click="router.visit(route('pengiriman.index'))">
    ...
    <div
        ...
        <div class="px-4 -mt-20 relative z-20 flex flex-col gap-5">
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                        ...
                        <div class="text-[10px] font-bold text-gray-500 dark:text-gray-400 tracking-widest uppercase">
                            Total Pendapatan
                        </div>
                    </div>
                    <div v-if="pendapatanDeltaText">
                        <span :class="pendapatanDeltaTone === 'pos'
                                ? 'bg-emerald-50 text-emerald-600'
                                : pendapatanDeltaTone === 'neg'
                                    ? 'bg-red-50 text-red-600'
                                    : 'bg-gray-50 text-gray-600'
                            " class="px-2 py-1 rounded-lg text-[10px] font-bold inline-flex items-center shadow-sm">
                            <i v-if="pendapatanDeltaTone === 'pos'" class="bi bi-arrow-up-short text-sm"></i>
                            <i v-else-if="pendapatanDeltaTone === 'neg'" class="bi bi-arrow-down-short text-sm"></i>
                            {{ pendapatanDeltaText.replace(' vs bulan lalu', '') }}
                        </span>
                    </div>
                </div>
                <div class="font-serif font-black text-gray-900 dark:text-white text-3xl tracking-tighter mt-1">
                    Rp
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2 bg-[#F0F4FF] dark:bg-indigo-900/20 rounded-[1.5rem] p-5 shadow-sm flex items-center justify-between animate-slide-up"
                ...
                <div class="col-span-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-[1.5rem] p-4 shadow-sm flex flex-col items-center justify-center text-center animate-slide-up"
                ...
                <div class="col-span-1 bg-red-50 dark:bg-red-900/20 rounded-[1.5rem] p-4 shadow-sm flex flex-col items-center justify-center text-center animate-slide-up"
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                        class="bg-gray-100 dark:bg-gray-700 text-gray-500 text-[9px] font-bold px-2 py-1 rounded-md uppercase">Bulan
                        Ini</span>
                </div>
                <div v-if="!mounted"
                    class="h-[250px] w-full bg-gray-100 dark:bg-gray-800 animate-pulse rounded-[1.5rem]"></div>
            ...
            <div class="mt-4">
                <div class="flex items-center justify-between mb-4 px-1">
                    ...
                    <div
                        ...
                        <div class="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse absolute"></div>
                        <div class="w-1.5 h-1.5 bg-green-500 rounded-full relative"></div>
                        <span
                            class="text-[9px] font-bold text-green-600 dark:text-green-400 uppercase tracking-widest">Live</span>
                    </div>
                </div>

                <div v-if="!mounted" class="space-y-3">
                    <div class="h-32 w-full bg-gray-200 dark:bg-gray-800 animate-pulse rounded-[1.5rem]"></div>
                    <div class="h-32 w-full bg-gray-200 dark:bg-gray-800 animate-pulse rounded-[1.5rem]"></div>
                </div>

                <!-- Desain Kartu Tracking Route ala Struk Tiket -->
                <div v-else class="flex flex-col gap-4">
                    <div v-for="p in latest.data" :key="'mob-t-' + p.id"
                        ...
                        @click="goDetail(p.id)">
                        <div class="p-4 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/30">
                            <span class="font-black text-primary text-base">{{
                                p.nomor_resi
                                }}</span>
                            <!-- Pill Badge kuat -->
                            <span :class="statusBadgeClass(p.status)" class="shadow-sm rounded-lg px-2 py-1">{{
                        ...
                        <div class="px-4 py-3 flex items-center justify-between">
                            ...
                            <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400"><i
                                    class="bi bi-clock me-1"></i>{{ fmtDateIso(p.created_at) }}</span>
                        </div>
                    </div>

                    <div v-if="latest.data.length === 0"
            ...
            <div class="mt-4">
                <button
                    ...
                    type="button" @click="router.visit(route('pengiriman.index'))">
